...
__global__ void kernel_fast_chamfer_distance_updateOutput_initializeIndices(int* d_indices, int indices_size) {
...
__global__ void kernel_fast_chamfer_distance_updateOutput_computeDistances(const float* d_input, const float* d_target, float* d_distances, int n_points) {
  int b = blockIdx.z;
  ...
  for (int d = 0; d < 3; d++) {
    ...
    d_distances[(b*n_points + n1)*n_points + n2] += difference*difference;
  }
}

__global__ void kernel_fast_chamfer_distance_updateOutput_computeLoss(float* d_distances, int* d_indices, float* d_loss, int n_points) {
...
float fast_chamfer_distance_updateOutput(const int batch_size, const int n_points, const float* d_input, const float* d_target, int* d_indices, bool size_average) {
  ...
  kernel_fast_chamfer_distance_updateOutput_initializeIndices<<<blocks, threads>>>(d_indices, indices_size);
  ...
  kernel_fast_chamfer_distance_updateOutput_computeDistances<<<grid, block>>>(d_input, d_target, d_distances, n_points);

  threads = max_threads/2;
  grid = dim3(ceil((float) n_points / (float) threads), batch_size);
  block = dim3(threads, 2);

  kernel_fast_chamfer_distance_updateOutput_computeLoss<<<grid, block>>>(d_distances, d_indices, d_loss, n_points);
